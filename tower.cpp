...
void Tower::aquire_target(){
  ...
  has_target=false;
    ...
    if(shoot_wait==0){
        ...
        for(int count=0;count<=(game->last_enemy+1);count++){//111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
        ...
        if(has_target){
            double dif=999;
            double dif_old=1000;
            for(steps_to_target=0;(dif>12)&&(dif_old>dif)&&(furthest_enemy+step_size*steps_to_target<game->last_waypoint);steps_to_target++){
                dif_old=dif;
                point_dif=(QPointF(x()+18,y()+18)-attack_dest);
                dif=(sqrt(pow(point_dif.x(),2)+pow(point_dif.y(),2)));
                dif = (dif - (steps_to_target*7));
